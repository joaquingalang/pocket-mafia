...
class GameSettingsBloc extends Bloc<GameSettingsEvent, GameSettingsState> {
  ...
  void _onRemovePlayer(
    GameSettingsRemovePlayer event,
    Emitter<GameSettingsState> emit,
  ) {
    emit(
      state.copyWith(
        names: List<String>.from(state.names)..removeAt(event.index),
      ),
    );
  }

  void _onSetDayDuration(
    GameSettingsSetDayDuration event,
    Emitter<GameSettingsState> emit,
  ) {
    emit(state.copyWith(dayDuration: event.duration));
  }

  void _onSetVoteDuration(
    GameSettingsSetVoteDuration event,
    Emitter<GameSettingsState> emit,
  ) {
    emit(state.copyWith(voteDuration: event.duration));
  }

  void _onSetNightDuration(
    GameSettingsSetNightDuration event,
    Emitter<GameSettingsState> emit,
  ) {
  ...
  void _onInitRoles(
    GameSettingsInitRoles event,
    Emitter<GameSettingsState> emit,
  ) {
    final mafiaCount = ((state.names.length + 1) / 4).floor().clamp(
      1,
      state.names.length - 1,
    );
    List<Roles> mafiaList = [Roles.detective, Roles.doctor];
    for (int i = 0; i < mafiaCount; i++) {
      mafiaList.add(Roles.mafia);
    }
    emit(state.copyWith(roles: [...mafiaList, ...state.roles]));
  }
